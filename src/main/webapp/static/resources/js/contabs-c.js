$(function() {
	...
	$(".nav-second-level li a").on(
			...
			function(e) {
				...
				/*if($(".skin-1 #side-menu>li:first-child ul").children("li").leng)*/
				e.stopPropagation();
				return false;
			})
	$(".nav-second-level li a").on(
			...
			function(e) {
				var con=0;
				var dataUrl = $(this).closest("a").attr('href');
				$('.skin-1 #side-menu>li:first-child li').find('a').each(function() {
				...
				if(con==$('.skin-1 #side-menu>li:first-child li').find('a').length){
					var html=$(this).closest("li").clone();
					html.find(".fa").removeClass("icon-comment addcomment").addClass("icon-reply removereplay");;
					$(".skin-1 #side-menu>li:first-child ul").append(html);
				}
				e.stopPropagation();
				return false;
			})
	function menuItem() {
		...
		if (dataUrl == undefined || $.trim(dataUrl).length == 0)
		 ...
		 if (!$(this).parent("li").hasClass("active")) {
		        $(this).closest(".navbar-default").find("li").removeClass("active");
		        $(this).parent("li").addClass("active");
		    }
		 $(this).closest(".nav-second-level").css({"background":"#fff"});
		// 选项卡菜单已存在
		$('.J_menuTab').each(
				function() {
...
//			dataUrl = dataUrl + "?random=" + timestamp;
